...
def timeseries_backtest(strategy_config, args, ts_backtest_cfg):
...
if __name__ == "__main__":
    ...
    for strategy_config in strategy_config_list:
        if 'interval' in strategy_config:
            if strategy_config['interval'] != args.interval:
                continue

        if args.target_strategy != strategy_config['strategy_name']:
            continue
        strategy_name = strategy_config['strategy_name']
        ts_backtest_cfg = ts_cfg[strategy_name + '.' + args.interval]

        timeseries_backtest(strategy_config, args, ts_backtest_cfg)
