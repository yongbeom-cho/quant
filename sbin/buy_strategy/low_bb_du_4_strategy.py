"""
Low BB DU 4 (Bollinger Band Down Up V2 - Reversed GC) 매수 전략

볼린저 밴드 하단에서 반등할 때 진입하는 전략 V2입니다.
기존 sbin/strategy/strategy.py의 low_bb_du_4 함수를 리팩토링한 구현입니다.

=============================================================================
low_bb_du_2와의 차이점:
- 스토캐스틱 골든크로스를 역방향으로 적용
- (stoch_d > stoch_k) 일 때를 골든크로스로 판단 (레거시 호환)
=============================================================================
...
3. cond3: 스토캐스틱 과매도 + 골든크로스 변형 (역방향, cond3_idx에 따라)
...
try:
    ...
    talib = None
    print("[WARNING] talib not installed. LowBBDU4 strategy requires talib.")

from .base import BaseBuyStrategy
from .position import PositionInfo


class LowBBDU4Strategy(BaseBuyStrategy):
    """
    볼린저 밴드 하단 반등 매수 전략 V2 (Low BB DU 4)
    
    low_bb_du_2와 동일하지만 스토캐스틱 골든크로스를 역방향으로 적용합니다.
    ...
    def calculate_signals(self, df: pd.DataFrame, cached_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        """
        if talib is None:
            raise ImportError("talib is required for LowBBDU4 strategy")
