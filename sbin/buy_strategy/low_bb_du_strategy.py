"""
Low BB DU (Bollinger Band Down Up) 매수 전략

볼린저 밴드 하단에서 반등할 때 진입하는 전략입니다.
기존 sbin/strategy/strategy.py의 low_bb_du 함수를 리팩토링한 구현입니다.
...
6. over_sell_golden_cross: 스토캐스틱 골든크로스 + 과매도 (올바른 방향)
...
try:
    ...
    talib = None
    print("[WARNING] talib not installed. LowBBDU strategy requires talib.")

from .base import BaseBuyStrategy
from .position import PositionInfo


class LowBBDUStrategy(BaseBuyStrategy):
    """
    볼린저 밴드 하단 반등 매수 전략 (Low BB DU)
    
    볼린저 밴드 하단에서 반등 시그널이 발생할 때 매수합니다.
    스토캐스틱 골든크로스를 사용하며, 올바른 방향의 GC를 적용합니다.
    (stoch_k > stoch_d 일 때 골든크로스)
    ...
    def calculate_signals(self, df: pd.DataFrame, cached_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        """
        if talib is None:
            raise ImportError("talib is required for LowBBDU strategy")
        ...
        # === 조건: 스토캐스틱 골든크로스 (올바른 방향) ===
        # stoch_k > stoch_d && 이전에는 stoch_d > stoch_k
