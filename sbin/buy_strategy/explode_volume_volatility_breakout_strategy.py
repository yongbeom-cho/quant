"""
Explode Volume Volatility Breakout 매수 전략

거래량 폭발 + 변동성 기반 돌파 전략입니다.
기존 sbin/strategy/strategy.py의 explode_volume_volatility_breakout 함수를 리팩토링한 구현입니다.

=============================================================================
전략 조건 (3가지 조건 모두 만족시 매수)
=============================================================================
1. volume_signal: 거래량이 N봉 trimmed mean 대비 vol_ratio배 이상
2. price_signal: 종가가 시가 + (평균 레인지 * k) 이상 (변동성 돌파)
...
class ExplodeVolumeVolatilityBreakoutStrategy(BaseBuyStrategy):
    """
    거래량 폭발 + 변동성 돌파 매수 전략
    ...
    def calculate_signals(self, df: pd.DataFrame, cached_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        # === 조건 2: 변동성 돌파 (close >= open + range * k) ===
        # 이전 봉의 레인지 평균 계산
    ...
    def create_position(
        ...
        return PositionInfo(
            ...
            entry_reason='explode_volume_volatility_breakout',
    ...
    def _get_required_config_keys(self) -> List[str]:
        """필수 설정 키 목록"""
        return ['window', 'vol_ratio', 'k', 'utr']
    
    @classmethod
    def get_default_config(cls) -> Dict[str, Any]:
        """기본 설정 반환"""
        return {
            'strategy_name': 'explode_volume_volatility_breakout',
            'window': 240,
